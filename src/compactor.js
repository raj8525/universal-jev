...
export async function compactMessages(client, messages, options = {}) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Anthropic style: toolUses and toolResults
    ...
    for (const tu of toolUses) {
      ...
      calls.push({
        ...
        resultText,
        pinned: isPinned || (resultIndex >= 0 && resultIndex >= total - preserveRecent),
      });
    }
  }
  ...
  cloned = cloned.filter((msg) => {
    ...
    if (msg.role === 'tool' && msg.tool_call_id) {
      ...
      if (act === 'truncate') {
        ...
        msg.content = `${orig.slice(0, truncateHeadChars)}\n[... ${orig.length - truncateHeadChars} chars pruned by Jev ...]`;
        return true;
      }
      keptCount++;
      return true;
    ...
    if (msg.toolResults && Array.isArray(msg.toolResults)) {
      msg.toolResults = msg.toolResults.filter((r) => {
        ...
        if (act === 'truncate') {
          truncatedCount++;
          const orig = r.text || '';
          r.text = `${orig.slice(0, truncateHeadChars)}\n[... ${orig.length - truncateHeadChars} chars pruned by Jev ...]`;
          return true;
        }
        if (act === 'drop') {
          droppedCount++;
